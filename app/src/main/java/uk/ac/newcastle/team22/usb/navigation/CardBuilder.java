...
public class CardBuilder {
    ...
    public static List<AbstractCardData> buildCards(List<Edge> edges, Context context) {
        ...
        for (int j = 0; j < parsedDirections.size(); j++) {
            // Check whether to add a tour card.
            ...
            // Check whether the user will need to change floors.
            if (parsedDirections.get(j).equals(Direction.LIFT_UP) || parsedDirections.get(j).equals(Direction.LIFT_DOWN) || parsedDirections.get(j).equals(Direction.STAIR_UP) || parsedDirections.get(j).equals(Direction.STAIR_DOWN)) {
                ...
                int absFloorDifference = Math.abs(floorDifference);

                // Check whether the user is to go up or down floors.
                String floorChangeText = floorDifference > 0 ? context.getString(R.string.navigationGoUp) : context.getString(R.string.navigationGoDown);
                if (absFloorDifference == 1) {
                    floorChangeText += " " + String.format(context.getString(R.string.navigationFloor), absFloorDifference);
                } else {
                    floorChangeText += " " + String.format(context.getString(R.string.navigationFloors), absFloorDifference);
                }
